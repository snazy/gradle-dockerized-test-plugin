...
class DockerizedTestPlugin implements Plugin<Project> {
    ...
    void configureTest(Project project, Test test) {
        ...
        test.doFirst {
            ...
            if (extension?.image)
            {
                LOGGER.debug("Applying 'docker' extension test executor to ${test.path}")
    ...
    def newProcessBuilderFactory(Project project, DockerizedTestExtension extension, Test test) {
        ...
        def execHandleFactory = [newJavaExec: { ->
            ...
            new DockerizedJavaExecHandleBuilder(extension, fileCollectionFactory, fileResolver, executor, buildCancellationToken, workerSemaphore, forkOptions, test, inetAddressFactory)
